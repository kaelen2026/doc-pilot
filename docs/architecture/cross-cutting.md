...
- Web 继续使用 `HttpOnly`、`Secure`、`SameSite` Cookie，不在浏览器 JavaScript 中读取
  session token。
- Apple 客户端在登录响应的 `set-auth-token` header 中接收 Better Auth Bearer
  session，保存到 Keychain；后续请求使用 `Authorization: Bearer <token>`。邮箱验证码
  (emailOTP)与原生 Sign in with Apple(`/sign-in/social` idToken 直登,见 ADR-012)
  共用这一收尾——都从该 header 取 Bearer 存 Keychain,不新增会话通路。
