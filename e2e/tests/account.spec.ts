 ...
 * 改昵称落库 → 主题三态切换 → 登录设备标本机 → 危险区注销按钮禁用。
 *
 * 无需真实模型/worker:仅用 web + api(/me、/me/usage)+ better-auth。
 */
test("用户中心:头像菜单进入 → 各分区就位 → 改昵称 → 换主题 → 会话 → 危险区禁用", async ({
  page,
}) => {
  ...
  await test.step("行内编辑昵称并落库", async () => {
    ...
    await expect(page.getByText("走查昵称")).toBeVisible({ timeout: 10_000 });
  ...
  await test.step("危险区的注销账户按钮为禁用占位", async () => {
    await expect(page.getByRole("button", { name: "注销账户" })).toBeDisabled();
  });
});
