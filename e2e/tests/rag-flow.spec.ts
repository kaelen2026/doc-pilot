...
test("注册登录 → 上传 → 就绪 → 提问 → 回答带引用 → 点击展开", async ({ page }) => {
  ...
  await test.step("文档列表轮询到「就绪」", async () => {
    ...
    const askLink = page.getByRole("main").locator(`a[href="/documents/${documentId}/chat"]`);
  ...
  await test.step("验证回答带有效引用(非拒答)并点击弹出原文", async () => {
    // 正文内嵌引用锚点「引用 1 …」出现 = 回答已完成且引用校验通过并落库。
    const citation = page.getByRole("button", { name: /^引用 1/ }).first();
    ...
    // 点击锚点 → 弹出引用 popover,内含可核对的原文 blockquote。
