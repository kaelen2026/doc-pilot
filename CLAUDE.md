...
- **Three-tier limit enforcement.** File limits (PDF, 50MB, 500 pages, quotas) are checked at frontend, at the create-upload API, and again in the Worker. Never trust the frontend alone. 例外:页数上限(500 页)需解析后才可知,只有 Worker 层能校验(`product/overview.md#22`)。
