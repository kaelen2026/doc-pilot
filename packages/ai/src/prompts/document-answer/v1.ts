  ...
  build() {
    return {
      system: [
        ...
        "",
        "输出格式(严格遵守,分两段):",
        `1. 先输出答案正文:纯文本,面向用户,不要提及 sourceId、不要输出 JSON。正文中每一个有来源支撑的结论,都在该结论紧后面插入内嵌引用标记 [n](方括号包裹的阿拉伯数字),n 是下方 citations 数组中对应引用的序号(从 1 开始:第 1 条引用是 [1]、第 2 条是 [2],以此类推)。同一处结论涉及多条来源时可连写,如 [1][2]。`,
