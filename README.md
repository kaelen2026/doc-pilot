...
> 本仓库已落地 Phase 1–7：一个可运行的 monorepo（`apps/` 下 Web / API / Worker 与 iOS 原生客户端，`packages/` 下共享库）。**设计文档仍是行为契约的权威参考**——实现或变更行为前，先读对应的 [`docs/`](docs/)。

## iOS 原生客户端

`apps/ios` 是 SwiftUI 编写的 iOS 客户端（iOS 26，Liquid Glass），直接复用公开 API，覆盖邮箱 OTP 登录、PDF 上传与缓存阅读、本地高亮、流式问答与引用跳页、搜索、通知和账户用量。
...
开发环境 API 地址配置在 `apps/ios/Config/Debug.xcconfig`；发布构建使用 `Release.xcconfig`。命令行门禁为 iOS Simulator 构建 + 测试，不依赖签名身份。

启动本地服务后，可运行真实 OTP 冒烟测试（Mailpit 自动取码）：

```bash
TEST_RUNNER_LIVE_SMOKE=1 xcodebuild -project apps/ios/DocPilot.xcodeproj \
...
- [移动端发版自动化](docs/runbooks/mobile-release.md) — iOS/Play/国内五商店的 tag 触发发版、Secret 清单、首跑确认项
