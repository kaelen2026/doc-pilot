...
> 本仓库已落地 Phase 1–7：一个可运行的 monorepo（`apps/` 下 Web / API / Worker 与 Apple 原生客户端，`packages/` 下共享库）。**设计文档仍是行为契约的权威参考**——实现或变更行为前，先读对应的 [`docs/`](docs/)。

## Apple 原生客户端

`apps/apple` 提供共用 SwiftUI 代码的 iOS 17+、iPadOS 17+ 与 macOS 14+ 客户端，直接复用公开 API，覆盖邮箱 OTP 登录、PDF 上传与缓存阅读、本地高亮、流式问答与引用跳页、搜索、通知和账户用量。
